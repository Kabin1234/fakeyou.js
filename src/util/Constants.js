...
module.exports.URL = {
  ...
  job: (token) => {
    return `${baseURL}/tts/job/${token}`;
  },
  model: (token) => {
    return `${baseURL}/tts/model/${token}`;
  },
  category: (token) => {
    return `${baseURL}/category/view/${token}`;
  },
  assignments: (token) => {
    return `${baseURL}/category/assignments/tts/${token}`;
  },
  result: (token) => {
    return `${baseURL}/tts/result/${token}`;
  },
  profile: (user) => {
    return `${baseURL}/user/${user}/profile`;
  },
  editProfile: (user) => {
    return `${baseURL}/user/${user}/edit_profile`;
  },
  ttsResults: (user) => {
    return `${baseURL}/user/${user}/tts_results`;
  },
  ttsModels: (user) => {
    return `${baseURL}/user/${user}/tts_models`;
  }
};
module.exports.Error = {
  ...
  failureResult: (token) => {
    return `The "${token}" request is a completed failure`;
  },
  optionNotFound: (option) => {
    return `Required option "${option}" re is missing`;
  },
  invalidType: (value, type) => {
    return `"${value}" option must be a${/^[aieou].*/i.test(type) ? 'n' : ''} ${type}`;
  },
  invalidOption: (option) => {
    return `The provided option "${option}" is invalid or not matches the verification`;
  },
  modelNotFound: (data) => `"${data}" model not found`,
  notCredentials: (option) => {
    return `The ${option} credential is missing`;
  }
};
