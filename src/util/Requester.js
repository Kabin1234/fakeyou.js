...
class Requester extends null {
  static async __getData(url, headers) {
    try {
      ...
      if (!e.response || !e.response.body)
        throw new FakeYouError(this, Constants.Error.webUnavailable, true);
      if (e.response.body.success == false) {
        throw new FakeYouError(
          this,
          e.response.body.error_message ?? e.response.body.error_reason,
          true
        );
      }
  ...
  static async __login(client) {
    ...
    try {
      ...
      if (body.success == true) {
        client.session.auth =
          headers['set-cookie'][0].match(/^\w+.=([^;]+)/)[1];
      }
    } catch (e) {
      if (!e.response || !e.response.body)
        throw new FakeYouError(this, Constants.Error.webUnavailable, true);
  ...
  static async __session(client) {
    ...
    if (!cookie)
      throw new FakeYouError(this, Constants.Error.notCredentials('auth'));
    const { logged_in, user } = await this.__getData(
      Constants.URL.session,
      Util.__getHeaders(client)
    );
    if (logged_in && Boolean(user)) {
      const info = await this.__getData(
        Constants.URL.profile(user.username),
        Util.__getHeaders(client)
      );
  ...
  static async __editUser(client, userObj, data) {
    if (!data)
      throw new FakeYouError(this, Constants.Error.optionNotFound('data'));
    if (!Util.checkType(data, 'object'))
      throw new FakeYouError(
        this,
        Constants.Error.invalidType('data', 'object')
      );
    const options = {
    ...
    const { success } = await this.__postData(
      Constants.URL.editProfile(client.user.username),
      options,
      Util.__getHeaders(client)
    );
    if (success) {
      const session = await Requester.__getData(
        Constants.URL.session,
        Util.__getHeaders(client)
      );
      const { user } = await Requester.__getData(
        Constants.URL.profile(userObj.username),
        Util.__getHeaders(client)
      );
  ...
  static async __createResult(client, token) {
    if (!Util.isToken(token, 'job'))
      throw new FakeYouError(this, Constants.Error.invalidToken);
    const isReady = (value) =>
      ['complete_success', 'complete_failure', 'dead'].includes(value);
    return await new Promise((resolve, reject) => {
      let interval = setInterval(async () => {
        const { state } = await this.__getData(
          Constants.URL.job(token),
          Util.__getHeaders(client)
        );
        if (isReady(state.status)) {
          ...
          if (state.status == 'complete_success') {
            ...
            reject(
              new FakeYouError(this, Constants.Error.failureResult(token), true)
            );
          }
        }
      }, 1000);
