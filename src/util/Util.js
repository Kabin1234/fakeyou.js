...
class Util {
  ...
  static verifyValue(value, data) {
    ...
    return (
      value.toLowerCase() == data.toLowerCase() ||
  ...
  static userPartialData(data) {
    const options = {
      ...
      display_name:
        data.maybe_creator_display_name ?? data.creator_display_name,
      user_token: data.maybe_creator_user_token ?? data.creator_user_token,
      gravatar_hash:
        data.maybe_creator_gravatar_hash ?? data.creator_gravatar_hash
    };
    return options;
  }
