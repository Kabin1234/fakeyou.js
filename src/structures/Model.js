...
class Model {
  constructor(client, data) {
    ...
    this.description = data.description_markdown
      ? data.description_markdown
      : null;
    this.categoryTokens =
      data.category_tokens ??
      data.categories.map((c) => c.category_token) ??
      null;
    this.category = Util.isNotEmptyObj(this.categoryTokens)
      ? this.client.categories.cache.filter((c) =>
          this.categoryTokens.includes(c.token)
        )
      : null;
  ...
  get partial() {
    return !(this.setVisibility && this.locale && this.lang);
  }
  ...
  isVisible() {
    if (!this.setVisibility) return null;
  ...
  async request(text) {
    ...
    let alreadyResult = this.client.results.cache.find(
      (r) =>
        r.text.toLowerCase() == text.toLowerCase() && r.modelToken == this.token
    );
    if (alreadyResult) return alreadyResult;
    let postData = await Requester.__postData(
      Constants.URL.inference,
      {
    ...
    const getData = await Requester.__createResult(
      this.client,
      postData.inference_job_token
    );
