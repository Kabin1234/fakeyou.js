...
class Group extends Map {
  first(count) {
    if (count == undefined) {
      ...
      if (!Util.checkType(count, 'number'))
        throw new FakeYouError(Constants.Error.invalidType('count', 'number'));
      if (count == 0) return this.last(1);
      else if (count < 0) return this.last(Math.abs(count));
      else {
        count = Math.min(this.size, count);
        return Array.from({ length: count }, () => this.values().next().value);
      }
    }
  }
  last(count) {
  ...
  some(fn, arg) {
    if (!Util.checkType(fn, 'function'))
      throw new FakeYouError(
        this,
        Constants.Error.invalidType('fn', 'function')
      );
    if (arg) {
      fn.bind(arg);
    }
