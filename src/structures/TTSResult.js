...
class TTSResult {
  constructor(client, data) {
    ...
    this.audioPath =
      data.maybe_public_bucket_wav_audio_path ??
      data.public_bucket_wav_audio_path ??
      null;
    this.spectrogramPath = data.public_bucket_spectrogram_path ?? null;
    this.modelToken = data.model_token ?? data.tts_model_token;
    this.model = this.modelToken
      ? (client.models.cache.get(this.modelToken) ??
        client.models.__add(Util.modelPartialData(data)))
      : null;
  ...
  get partial() {
    return !(this.spectrogramPath && this.size && this.duration);
  }
  ...
  audioURL() {
    return this.audioPath ? Constants.URL.cdn + this.audioPath : null;
  }
  spectrogramURL() {
    return this.spectrogramPath
      ? Constants.URL.cdn + this.spectogramPath
      : null;
  }
  resultURL() {
    return this.token
      ? `${Constants.URL.webPage}/tts/result/${this.resultToken}`
      : null;
  }
  async getAudio() {
