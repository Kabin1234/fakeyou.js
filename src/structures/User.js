...
class User {
  constructor(client, data) {
    ...
    this.badges = Util.isNotEmptyObj(data.badges)
      ? new Badges(this.client, this, data.badges)
      : null;
  ...
  get partial() {
    return !(this.createdAt && this.ttsVisibility && this.w2lVisibility);
  }
  ...
  isVisible(isw2l) {
    if (!Util.checkType(isw2l, 'boolean', true))
      throw new FakeYouError(Constants.Error.invalidOption('isw2l', 'boolean'));
    if (isw2l) {
      if (this.w2lVisibility) return null;
      return this.w2lVisibility == 'public' ? true : false;
    } else {
      if (this.ttsVisibility) return null;
