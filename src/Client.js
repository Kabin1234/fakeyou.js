...
class Client {
  ...
  searchModel(query) {
    ...
    if (!Util.checkType(query, 'string'))
      throw new FakeYouError(
        this,
        Constants.Error.invalidType('query', 'string')
      );
    const search = this.models.cache.filter(
      (m) => Util.verifyValue(m.title, query) || Util.verifyValue(m.name, query)
    );
    return search;
  }

  async queue() {
    const queueData = await Requester.__getData(
      Constants.URL.queue,
      Util.__getHeaders(this)
    );
    let options = {
  ...
  async makeTTS(model, text) {
    if (!model)
      throw new FakeYouError(this, Constants.Error.optionNotFound('model'));
    if (!text)
      throw new FakeYouError(this, Constants.Error.optionNotFound('text'));
    if (!Util.checkType(text, 'string'))
      throw new FakeYouError(
        this,
        Constants.Error.invalidType('text', 'string')
      );
    const findModel = this.models.resolve(model);
    if (!findModel)
      throw new FakeYouError(this, Constants.Error.modelNotFound(model));
    return findModel.request(text);
  }
  async leaderboard() {
    const getData = await Requester.__getData(
      Constants.URL.base + '/leaderboard',
      Util.__getHeaders(this)
    );
  ...
  async fetchUser(query) {
    if (!query)
      throw new FakeYouError(this, Constants.Error.optionNotFound('query'));
    if (query instanceof User) {
      ...
      const { user } = await Requester.__getData(
        Constants.URL.profile(query),
        Util.__getHeaders(this)
      );
  ...
  __patchOptions(options = {}) {
    if ('token' in options) {
      if (!Util.checkType(options.token, 'string'))
        throw new FakeYouError(
          this,
          Constants.Error.invalidType('token', 'string')
        );
      this.session.token = options.token ?? null;
    }
    if ('usernameOrEmail' in options) {
      if (!Util.checkType(options.usernameOrEmail, 'string'))
        throw new FakeYouError(
          this,
          Constants.Error.invalidType('username or email', 'string')
        );
      this.session.usernameOrEmail = options.usernameOrEmail ?? null;
    }
    if ('password' in options) {
      if (!Util.checkType(options.password, 'string'))
        throw new FakeYouError(
          this,
          Constants.Error.invalidType('password', 'string')
        );
